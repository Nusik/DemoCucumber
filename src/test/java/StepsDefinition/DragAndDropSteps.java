...
public class DragAndDropSteps {
    ...
    public void successMessageIsNotShown() {
        dragAndDropPage.getPerfectMessage().shouldNotBe(Condition.visible); //exist ?
    }

    @When("user drop debit account {string}")
    public void userDropDebitAccount(String button) {
        assertEquals(button, dragAndDropPage.getBank().getText());
        dragAndDropPage.dragBankToDebitAccount();
    }

    @And("user drop debit amount {string}")
    public void userDropDebitAmount(String button) {
        assertEquals(button, dragAndDropPage.getFiveThousands().getText());
dragAndDropPage.dragFiveThousandToDebitAmount();
    }

    @And("user drop credit account {string}")
    public void userDropCreditAccount(String button) {
        assertEquals(button, dragAndDropPage.getSales().getText());
        dragAndDropPage.dragSalesToCreditAccount();
    }

    @And("user drop credit amount {string}")
    public void userDropCreditAmount(String button) {
        assertEquals(button, dragAndDropPage.getFiveThousands().getText());
